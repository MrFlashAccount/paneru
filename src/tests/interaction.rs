...
use crate::ecs::{
    ActiveWorkspaceMarker, AxPositionWrite, FocusedMarker, LayoutPosition, NativeFullscreenMarker,
    PendingScrollVerification, Position, PreManagedFrame, ResizeMarker, Scrolling, Unmanaged,
    VerifyWindowPosition, WindowDisposition, layout::LayoutStrip,
};
use crate::ecs::{RepositionMarker, SpawnWindowTrigger};
use crate::events::Event;
use crate::manager::{Origin, Size, Window};
use crate::platform::{Modifiers, ProcessSerialNumber};
...
#[test]
fn external_move_during_long_active_swipe_survives_release_and_deadlines() {
    ...
    let strip = {
        ...
        strips.single(world).expect("one active layout strip")
    };
    let expected = harness.world().get::<Position>(window).unwrap().0;
    harness.world().entity_mut(window).insert((
        PendingScrollVerification,
        VerifyWindowPosition::default(),
        AxPositionWrite::new(expected),
        RepositionMarker(expected),
    ));
    harness.world().entity_mut(strip).insert(Scrolling {
        gesture_active: true,
    ...
    assert!(harness.world().entity(strip).contains::<Scrolling>());

    let external = Origin::new(-260, TEST_MENUBAR_HEIGHT);
    harness.mock_state.reset_reposition_writes(0);
    harness.mock_state.os_move_window(0, external);
    ...
    assert_eq!(world.get::<Position>(window).unwrap().0, external);
    assert!(!world.entity(window).contains::<AxPositionWrite>());
    assert!(!world.entity(window).contains::<PendingScrollVerification>());
    assert!(!world.entity(window).contains::<VerifyWindowPosition>());
    assert!(!world.entity(window).contains::<RepositionMarker>());
    assert!(!world.entity(strip).contains::<Scrolling>());
    assert_eq!(harness.mock_state.reposition_writes(0), 0);
}

#[test]
fn authored_a_b_delayed_a_across_registered_schedule_turns_preserves_latest_target() {
    let mut harness = TestHarness::new().with_windows(1);
    harness.run(vec![Event::MenuOpened { window_id: 0 }]);
    ...
    let authored_a = Origin::new(-100, TEST_MENUBAR_HEIGHT);
    let target_b = Origin::new(-300, TEST_MENUBAR_HEIGHT);
    let mut history = AxPositionWrite::new(authored_a);
    history.record(target_b);
    harness
        .world()
        .get_mut::<Position>(window)
        .unwrap()
    ...
    harness.mock_state.os_move_window(0, target_b);
    for event in harness.mock_state.drain_events() {
        harness.app.world_mut().write_message::<Event>(event);
    }
    harness.app.update();
    assert_eq!(harness.world().get::<Position>(window).unwrap().0, target_b);
    assert!(harness.world().entity(window).contains::<AxPositionWrite>());
    let writes_after_latest_ack = harness.mock_state.reposition_writes(0);

    harness.mock_state.os_move_window(0, authored_a);
    for event in harness.mock_state.drain_events() {
        harness.app.world_mut().write_message::<Event>(event);
    }
    harness.app.update();
    ...
    assert!(harness.world().entity(strip).contains::<Scrolling>());
    assert_eq!(
        harness.mock_state.reposition_writes(0),
        writes_after_latest_ack,
        "delayed authored A must not write back over target B"
    );
}

fn assert_geometry_ownership_cleared(world: &World, window: Entity) {
    let window = world.entity(window);
    assert!(!window.contains::<AxPositionWrite>());
    assert!(!window.contains::<PendingScrollVerification>());
    assert!(!window.contains::<VerifyWindowPosition>());
    assert!(!window.contains::<RepositionMarker>());
    assert!(!window.contains::<ResizeMarker>());
}

#[test]
fn due_verifier_cannot_override_app_exit_centering() {
    let mut harness = TestHarness::new().with_windows(1);
    harness.run(vec![Event::MenuOpened { window_id: 0 }]);
    ...
    let stale_target = Origin::new(-500, TEST_MENUBAR_HEIGHT);
    harness.world().entity_mut(window).insert((
        AxPositionWrite::new(stale_target),
        PendingScrollVerification,
        VerifyWindowPosition::default(),
        RepositionMarker(stale_target),
        ResizeMarker(Size::new(300, 300)),
    ));
    harness.world().entity_mut(strip).insert((
        Scrolling {
            gesture_active: true,
            is_user_swiping: true,
            ..Default::default()
    ...
    harness
        .app
        .world_mut()
        .write_message::<AppExit>(AppExit::Success);
    harness.app.update();

    let centered = harness
        .world()
        .get::<Window>(window)
        .expect("window survives cleanup turn")
        .frame()
        .min;
    let writes_after_centering = harness.mock_state.reposition_writes(0);
    assert_eq!(writes_after_centering, 1);
    assert_geometry_ownership_cleared(harness.app.world(), window);
    ...
    assert_eq!(
        harness.mock_state.reposition_writes(0),
        writes_after_centering
    );
}

#[test]
fn active_scroll_to_native_fullscreen_cancels_deferred_geometry() {
    const FULLSCREEN_WORKSPACE_ID: WorkspaceId = TEST_WORKSPACE_ID + 101;
    ...
    let focused = {
        let world = harness.world();
        let mut query = world.query_filtered::<Entity, With<FocusedMarker>>();
        query.iter(world).collect::<Vec<_>>()
    };
    for entity in focused {
        harness.world().entity_mut(entity).remove::<FocusedMarker>();
    }
    let target = harness.world().get::<Position>(window).unwrap().0;
    harness.world().entity_mut(window).insert((
        AxPositionWrite::new(target),
        PendingScrollVerification,
        VerifyWindowPosition::default(),
        RepositionMarker(target),
        ResizeMarker(Size::new(300, 300)),
    ));
    harness.world().entity_mut(strip).insert((
        Scrolling {
    ...
    harness
        .app
        .world_mut()
        .write_message::<Event>(Event::SpaceChanged);
    for _ in 0..5 {
        harness.app.update();
    }
...
#[test]
fn active_scroll_to_passthrough_cancels_deferred_geometry() {
    let mut harness = TestHarness::new().with_windows(1);
    harness.run(vec![Event::MenuOpened { window_id: 0 }]);
    let window = find_window_entity(0, harness.world());
    let strip = {
    ...
    let writes_after_transition = harness.mock_state.reposition_writes(0);
    for _ in 0..4 {
        harness.app.update();
    }
    assert_eq!(
        harness.mock_state.reposition_writes(0),
        writes_after_transition
    );
}

#[test]
fn application_termination_despawns_authored_history_with_window() {
    let mut harness = TestHarness::new().with_windows(1);
    harness.run(vec![Event::MenuOpened { window_id: 0 }]);
    let window = find_window_entity(0, harness.world());
    let target = harness.world().get::<Position>(window).unwrap().0;
    harness
        .world()
        .entity_mut(window)
        .insert(AxPositionWrite::new(target));

    harness
        .app
        .world_mut()
        .write_message::<Event>(Event::ApplicationTerminated {
            psn: ProcessSerialNumber {
                high: 0,
                low: TEST_PROCESS_ID.cast_unsigned(),
            },
        });
    harness.app.update();

    assert!(
        harness.app.world().get_entity(window).is_err(),
        "authored history is entity-owned and must disappear with the terminated app"
    );
}
